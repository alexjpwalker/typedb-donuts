...
curl -X POST http://localhost:3000/api/outlets \
  -H "Content-Type: application/json" \
  -d '{"outletId":"outlet-1","outletName":"Downtown Donuts","location":"123 Main St","balance":10000,"marginPercent":20}'

echo -e "\nUptown Bakery: 35% margin (Premium pricing)"
curl -X POST http://localhost:3000/api/outlets \
  -H "Content-Type: application/json" \
  -d '{"outletId":"outlet-2","outletName":"Uptown Bakery","location":"456 Oak Ave","balance":10000,"marginPercent":35}'

echo -e "\nSweet Treats Shop: 50% margin (Luxury brand)"
curl -X POST http://localhost:3000/api/outlets \
  -H "Content-Type: application/json" \
  -d '{"outletId":"outlet-3","outletName":"Sweet Treats Shop","location":"789 Maple Dr","balance":10000,"marginPercent":50}'
